    ...
    def draw(self, context):
        ...
        if bpy.context.object and bpy.context.object.type == 'MESH':
            if not bpy.data.filepath:
                ...
                elif ('Matcap' in bpy.data.materials and not os.path.isfile(mydirectory + "backup_setup")) or ('Matcap' in bpy.data.materials and ob.active_material.name != "Matcap" and os.path.isfile(mydirectory + "backup_setup")):
...
class  SetupMatcap(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if 'Matcap' in bpy.data.materials and not os.path.isfile(mydirectory + "backup_setup"):                    
            addon_name = "mesh_check"
            if addon_name in [addon.module for addon in bpy.context.user_preferences.addons]:
                if bpy.context.object.material_slots:
                    if bpy.context.object.active_material.name == "Quads":
